...
const CODE_PT: Record<string, string> = {
  ...
  AGRUPAMENTO_REMOVER_ULTIMO: 'Não é possível remover o último título do agrupamento — reverta o agrupamento.',
  ...
  CONTA_CONTABIL_INVALIDA: 'Conta de lançamento contábil inválida — informe uma conta analítica de empresa.',
  CONTA_OPERADOR_DUPLICADO: 'Operador repetido na liberação da conta.',
  ...
  PROMOCAO_ORIGEM_NAO_SUPORTADA: 'Esta mecânica de promoção ainda não foi implementada — use Preço Fixo, Desconto Fixo, Desconto Variável, Código Promocional, Combo, Leve Pague, Categoria, Atacarejo ou Bonificação.',
  ...
  // CNAB de cobrança (FRMCONFBOLETO): boleto + remessa
  BOLETO_JA_ENVIADO: 'Há título já enviado ao banco (ou cancelado) — ele não pode ser reemitido.',
  BOLETO_NAO_EMITIDO: 'Emita o boleto antes de gerar a remessa (a remessa leva só título com boleto emitido).',
  REMESSA_VALOR_INVALIDO: 'Título com valor zerado ou negativo não pode ir na remessa.',
  ...
  BOLETO_BANCO_NAO_SUPORTADO: 'O código de barras deste banco ainda não é gerado (neste corte, Itaú e Banco do Brasil).',
  BOLETO_CAMPO_LIVRE_INVALIDO: 'Os dados da conta não formam o campo livre do código de barras — confira carteira, agência, conta e convênio.',
  BOLETO_BARRAS_INVALIDO: 'Código de barras inválido (deve ter 44 dígitos).',
  ...
  RETORNO_SEM_PAGAMENTO: 'Não há boleto com valor recebido neste arquivo de retorno.',
  RETORNO_TITULOS_NAO_ENCONTRADOS: 'Os boletos do retorno não foram encontrados no sistema, ou já foram baixados.',
  AGENCIA_INVALIDA: 'A agência da configuração bancária está inválida (são 4 dígitos).',
  EMPRESA_CEDENTE_NAO_ENCONTRADA: 'A empresa cedente da configuração bancária não existe.',
  CONTA_INVALIDA_ITAU: 'Número de conta inválido para o Itaú (são 5 dígitos + dígito verificador).',
  CNPJ_CEDENTE_INVALIDO: 'O CNPJ/CPF da empresa (cedente) está inválido no cadastro — corrija antes de gerar a remessa.',
  CNAB_CAMPO_OBRIGATORIO: 'Falta um dado obrigatório da remessa (veja o campo indicado) no cadastro.',
  CNAB_CAMPO_LONGO: 'Um dado da remessa não cabe no campo do arquivo (veja o campo e o máximo indicados).',
  CNAB_INVALIDO: 'O arquivo de remessa saiu inválido e não foi gravado — confira os dados dos títulos e do cadastro.',
  LAYOUT_NAO_SUPORTADO: 'Layout de remessa ainda não suportado (este corte gera o CNAB 400 do Itaú).',
  BANCO_NAO_SUPORTADO: 'Banco ainda não suportado na remessa (este corte gera o CNAB 400 do Itaú).',
  REMESSA_NAO_ENCONTRADA: 'Remessa não encontrada nesta empresa.',
  REMESSAS_DO_DIA_ESGOTADAS: 'Limite de remessas para hoje atingido (100 arquivos no mesmo dia).',
  ...
  ANALISE_JA_FINALIZADA: 'Esta análise já foi finalizada.',
  ANALISE_SEM_PERMISSAO_LIBERAR: 'Você não pode liberar esta análise: ela só é liberada por quem fez o pedido ou por um operador master.',
  ANALISE_VARIOS_COMPRADORES: 'Apenas o operador master pode liberar: os pedidos desta análise são de compradores diferentes.',
  ANALISE_EXIGE_FINANCEIRO: 'Para liberar a análise com divergência é preciso gerar o financeiro (o título da diferença).',
  ANALISE_DIFERENCA_ZERADA: 'A diferença apurada é zero — não há financeiro a gerar.',
  ...
  SITUACAO_CFOP_DUPLICADO: 'CFOP repetido na situação do documento.',
  SITUACAO_CFOP_INEXISTENTE: 'CFOP não cadastrado.',
  SITUACAO_CFOP_TIPO_DIFERENTE: 'O CFOP é de outro tipo (entrada/saída) que a situação do documento.',
  SITUACAO_CC_INEXISTENTE: 'Centro de custo não cadastrado.',
  SITUACAO_PARCEIRO_INEXISTENTE: 'Parceiro não cadastrado.',
  SITUACAO_EM_USO: 'A situação do documento está em uso (contábil, NF, contas a pagar/receber ou scrap) e não pode ser excluída.',
