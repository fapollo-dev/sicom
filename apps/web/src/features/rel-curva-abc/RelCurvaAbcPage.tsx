...
interface Linha {
  id?: number | string | null; idproduto?: number | null; codbarra?: string; descricao?: string;
  unidade?: string; departamento?: string; codparceiro?: number | null;
...
export function RelCurvaAbcPage() {
  ...
  const consultar = async () => {
    ...
    setBusy(true);
    try {
      // o «ranking por quantidade» (rel 18) é outro endpoint: não tem curva nenhuma, só ordena por qtde.
      const rota = modo === 'QUANTIDADE' ? '/relatorios/curva-abc/quantidade' : '/relatorios/curva-abc/consultar';
      const r = await req<{ linhas: Linha[]; totais: Totais; filtro: Filtro }>(rota, {
        dtini, dtfim, produto: produto || undefined, exibirFilhos,
        dimensao: modo === 'QUANTIDADE' ? undefined : modo,
      });
  ...
  // o rel 18 não classifica nada — a tela some com as colunas de curva em vez de exibi-las vazias
  const ranking = modo === 'QUANTIDADE';
  const rotulo = modo === 'CLIENTE' ? 'Cliente' : modo === 'FORNECEDOR' ? 'Fornecedor' : 'Produto';

  return (
    <div className="flex flex-col gap-gp-md p-pad-md">
      <PageHeader title={ranking ? 'Ranking de produtos por quantidade vendida' : `Curva ABC por ${rotulo.toLowerCase()}`} />
      ...
      <div className="flex flex-wrap items-end gap-gp-sm rounded-radius-md border border-border bg-bg-surface p-pad-md">
        ...
        <div className="w-56"><SelectField label="C&urva por" value={modo} onChange={setModo} options={[
          { value: 'PRODUTO', label: 'Produto (rel 09)' },
          { value: 'CLIENTE', label: 'Cliente (rel 10)' },
          { value: 'FORNECEDOR', label: 'Fornecedor (rel 11)' },
          { value: 'QUANTIDADE', label: 'Ranking por quantidade (rel 18)' },
        ]} /></div>
        <div className="w-56"><Field label="&Produto" value={produto} onChange={(e) => setProduto(e.target.value)} placeholder="parte da descrição" /></div>
      ...
      {filtro?.sem_curva_configurada && !ranking && (
        ...
        <div className="flex flex-wrap gap-gp-sm">
          {['A', 'B', 'C'].map((f) => (
            <div key={f} className="flex-1 min-w-36 rounded-radius-md border border-border bg-bg-surface p-pad-sm">
              <div className="text-body-xs text-fg-muted">Faixa {f} — {totais.faixas?.[f]?.linhas ?? 0} {rotulo.toLowerCase()}s</div>
        ...
        <table className="w-full text-body-sm">
          <thead>
            <tr className="text-left text-fg-muted">
              {!ranking && <th className="p-pad-xs">ABC</th>}
              <th className="p-pad-xs">{rotulo}</th>
              <th className="p-pad-xs text-right">Qtde</th>
              {!ranking && <>
                <th className="p-pad-xs text-right">Vr. venda</th>
          ...
          <tbody>
            {linhas.map((l, i) => (
              <tr key={`${l.id ?? l.idproduto ?? i}`} className="border-t border-border">
                ...
                <td className="p-pad-xs">
                  {l.descricao}
                  {(l.codbarra || l.codparceiro != null) && (
                    <><br /><span className="text-fg-muted">
                      {l.codbarra ? `${l.codbarra} · ${l.unidade ?? ''} · ${l.departamento ?? '—'}` : `cód. ${l.codparceiro}`}
                    </span></>
                  )}
                </td>
            ...
            {!linhas.length && <tr><td colSpan={ranking ? 2 : 8} className="p-pad-md text-fg-muted">Informe o período e consulte.</td></tr>}
          </tbody>
        </table>
      </div>
