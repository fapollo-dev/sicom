 ...
 * Dossiê: `uConsRCBbx.md`. Espelho da página do A Pagar, com a observação editável da baixa.
 *
...
interface Titulo extends Record<string, unknown> { codrcb: number; codrcbbx: number; duplicata: string; cliente: string; diasAtraso: number; obs_editavel: string | null; emissao: string; vencimento: string; data_pagamento: string; valorDocumento: number; valorpg: number; juros: number; operador_baixa: string | null; revertida: boolean; revertida_em: string | null; revertida_por: string | null }
...
export function ConsRcbBxPage() {
  ...
  const buscar = async () => {
    ...
    try {
      ...
      setDet(null);
      setLotes((await pedir<{ lotes: LoteResumo[] }>(`${BASE}/cobranca/cons-rcb-bx/lotes?${q}`)).lotes);
  ...
  const reverter = async () => {
    ...
    setOcupado(true);
    try {
      const r = await pedir<{ titulosRevertidos: number; contraMovimentos: number }>(`${BASE}/cobranca/cons-rcb-bx/${det.lote}/reverter`, { method: 'POST' });
  ...
  return (
    <div className="flex flex-col gap-gp-md">
      <PageHeader title="Baixas do contas a receber" />

      <section className="rounded-radius-md border border-border bg-bg-surface p-pad-md">
        <p className="mb-form-gap text-body-sm text-fg-muted">
          Os lotes de recebimento do período. Abra um lote para ver os títulos recebidos (com os dias de atraso), o movimento da conta bancária e editar a observação de cada baixa;
          ...
          <table className="w-full min-w-[900px] border-collapse text-body-sm">
            <thead>
              <tr className="border-b border-border text-left text-fg-muted">
                ...
                <th className="p-pad-xs">Clientes</th><th className="p-pad-xs text-right">Pago</th><th className="p-pad-xs text-right">Juros</th>
            ...
            <tbody>
              ...
              {lotes.map((l) => (
                <tr key={l.lote} onClick={() => void abrir(l.lote)} className={`cursor-pointer border-b border-border hover:bg-bg-subtle ${det?.lote === l.lote ? 'bg-bg-subtle font-semibold' : ''}`}>
                  ...
                  <td className="p-pad-xs">{l.clientes > 1 ? `${l.clientes} — ${l.razoes}` : l.razoes}</td>
      ...
      {det && (
        <>
          ...
          <div className="overflow-x-auto rounded-radius-md border border-border bg-bg-surface">
            <h4 className="p-pad-xs text-body-sm font-semibold">Títulos recebidos</h4>
            <table className="w-full min-w-[900px] border-collapse text-body-sm">
              <thead>
                <tr className="border-b border-border text-left text-fg-muted">
                  <th className="p-pad-xs">Documento</th><th className="p-pad-xs">Cliente</th><th className="p-pad-xs">Emissão</th><th className="p-pad-xs">Vencimento</th><th className="p-pad-xs text-right">Atraso</th>
                  ...
                  <th className="p-pad-xs">Operador</th><th className="p-pad-xs">Situação</th><th className="p-pad-xs">Observação</th>
                </tr>
              </thead>
              <tbody>
                {det.titulos.map((t) => (
                  <tr key={t.codrcb} className={`border-b border-border ${t.revertida ? 'text-fg-muted' : ''}`}>
                    <td className="p-pad-xs">{t.duplicata}</td><td className="p-pad-xs">{t.cliente}</td>
                    <td className="p-pad-xs">{dataBr(t.emissao)}</td><td className="p-pad-xs">{dataBr(t.vencimento)}</td><td className={`p-pad-xs text-right tabular-nums ${t.diasAtraso > 0 ? 'text-fg-danger' : ''}`}>{t.diasAtraso > 0 ? `${t.diasAtraso} d` : ''}</td><td className="p-pad-xs">{dataBr(t.data_pagamento)}</td>
                    ...
                    <td className="p-pad-xs"><input className="w-56 rounded-radius-sm border border-border bg-bg-surface p-pad-xs text-body-sm" defaultValue={t.obs_editavel ?? ''} placeholder="observação" onBlur={(e) => void gravarObs(t.codrcbbx, e.target.value)} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
