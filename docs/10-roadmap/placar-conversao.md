...
| `BALANCOITENS` | 980.574 | 2020-08 → **28/01/2026** (via `BALANCO`) | `uInventario.pas` + `uCadBalanco.pas` + `udmInventario.dfm` | ✅ **conferido: é LACUNA em épico migrado, não tabela órfã** (dossiê `uInventario-balanco.md`). A mig 090 migrou `INVENTARIO_LIVRO`+`INVENTARIO`; o **balanço** (a foto: 24 cabeçalhos × ≈43 mil itens = a base inteira de produtos, 96% com qtde 0) e os **5 comandos** do popup que giram nele (Importar Balanço · Importar Balanço e Atualizar Estoque · Gerar Balanço a partir do Inventário · Sincronizar Inventário (Entradas−Saídas) · Relatório Diferença do Balanço para Estoque) não entraram. Vivo: última foto **28/01/2026**; `FRMCADBALANCO` tem 6 opções/16 operadores |
...
**Fila por dado ZERADA (2026-08-20)** — o último item com volume, `NF_PROD_LOTE`, entrou no corte-1 (mig 169): é o grupo `rastro` do XML da NF-e.
O próximo alvo saiu do próprio recon do balanço: **INVENTÁRIO ROTATIVO** (`FRMRELINVENTARIOROTATIVO`, dossiê `uRelatorioInventarioRotativo.md`) — `INVENTARIO_ROTATIVO` tem movimento até **31/07/2026**, a tabela mais recente de toda a varredura (mais que a `NFE_INUTILIZADA`, 28/05/2026): 82 lotes, 4 empresas, máquina de estado por LINHA (`OPERACAO='ABERTO'` e depois outra `'FECHADO'`, não UPDATE), coletor com importação de arquivo, 5 configs próprias e **duas pontes de dinheiro** — NF de perdas e NF de sobras carimbadas pela tela de NF (`uNF.pas:5267/5280`), com gate anti-reimporte e estorno no cancelamento (`udmNF.pas:3418-3457`). E ele **explica** de onde vêm as fotos `BALANCO_GERAL_*` do golden: são do inventário GERAL (`TIPO='G'`) — só que `CODBALANCO_INICIAL`/`_FINAL` **não existem em nenhum fonte do repo clonado** (lição 35: regra de versão mais nova ⇒ colunas entram para a carga, sem lógica).
