...
> O DataScience/IA da iGreen é **portado** (não reescrito do zero) para o cliente "surfar a onda da IA" com **os dados deles** — que têm muito valor de varejo. **Strip de TODO vínculo iGreen** (links, dados, segredos, tenants, prompts, knowledge) e **adaptação ao domínio de supermercado**. É **FASE POSTERIOR** (Fase 6 do roadmap): só depois do core migrado e dos relatórios/BI baseline no ar. Não é prioridade inicial.

## Pré-requisitos de leitura

- [../00-orientation/canonical-decisions.md](../00-orientation/canonical-decisions.md) — **ADR-013** (DataScience é fork limpo, sem vínculo iGreen).
...
O **motor** (infra de RAG/semântica) é genérico e portável. O **conteúdo** (dados, prompts, knowledge, tenants, segredos) é **100% iGreen** e **não** vai junto.
...
| Few-shots / exemplos | **NÃO** | são casos iGreen reais — reconstruir com casos do cliente |
| Knowledge base / regras | **NÃO** | conhecimento iGreen (comissão, bônus, rede) — irrelevante e vazante |
| Prompts / system prompts | **NÃO** | persona, tenant, domínio iGreen embutidos — reescrever |
| Tenants / IDs / dados | **NÃO** | dado de produção iGreen — **proibido** sair |
...
> **Regra:** porta-se o **encanamento**, não a **água**. Tudo que carrega dado, conhecimento, identidade ou segredo iGreen fica para trás.

---

## 4) CHECKLIST de strip iGreen (DataScience)
...
- [ ] **Nenhum dump/seed/fixture** com dados de produção iGreen (clientes, consultores, vendas, comissão).
- [ ] **Embeddings/índices vetoriais** reconstruídos do zero — não importar índice iGreen (carrega o conteúdo original).
- [ ] **Tabelas de exemplo / golden datasets** trocadas por casos do cliente.
- [ ] **PII** de qualquer pessoa iGreen (nomes, telefones, e-mails, IDs) zerada — LGPD se aplica a dado de terceiro também.

### 4.2 Prompts, knowledge e few-shots

- [ ] **System prompts** reescritos para domínio supermercado; sem persona/tenant/marca iGreen.
- [ ] **Knowledge rules** iGreen (comissão, bônus, rede, saldo, devolutiva) **removidas** — reescrever com regras de varejo do cliente.
- [ ] **Few-shots** reconstruídos com perguntas/respostas do domínio do cliente.
- [ ] **Exemplos de SQL / esquema** apontando para o schema do cliente, não para `IGREEN`/`IGREEN_HUB`.
...
- [ ] **Tokens/credenciais** iGreen (ex.: tokens `ign_live_*`, DS_AUTH, Knowledge secret) **removidos e rotacionados**.
- [ ] **DSN / connection strings** apontando para bancos iGreen (`162.x`, `IGREEN`, `IGREEN_HUB`).
- [ ] **Refs de tenant / IDs de conexão** (WABA, phone IDs, tenant maps) iGreen apagados.
- [ ] **Endpoints** (DS API base, Hub, Knowledge dashboard) trocados pelos do cliente.
- [ ] **Webhooks / callbacks** com domínio iGreen.

### 4.4 Código, config e histórico

- [ ] **Comentários/TODO/nomes de módulo** referenciando produtos iGreen.
- [ ] **Variáveis de ambiente** (`.env*`) sem chave iGreen, mesmo "de exemplo".
- [ ] **Histórico git** limpo (`--orphan`/export sem `.git`) — não carregar o log do repo iGreen.
- [ ] **CI/CD secrets e workflows** desvinculados da org iGreen.
...
```bash
#!/usr/bin/env bash
# scripts/check-no-igreen-ds.sh — gate de CI para o port de DataScience
set -euo pipefail
# marca, domínios, knowledge iGreen, tokens, bancos
PATTERNS='igreen|igreenenergy|ign_live_|IGREEN_HUB|\bIGREEN\b|knw_|comiss|bonus|consultor|licenciado|hub\.igreen|162\.141\.111\.96|V_EXTRATOBONUS|GET_DESCENDANTS'
HITS=$(grep -rniE "$PATTERNS" src/ prompts/ knowledge/ config/ .env* || true)
if [ -n "$HITS" ]; then
  echo "❌ STRIP iGREEN (DS) FALHOU:"; echo "$HITS"; exit 1
fi
echo "✅ strip iGreen (DataScience) OK"
```

> **Mais grave que o DS visual:** aqui o vazamento é **dado/segredo/conhecimento de terceiro**, não só cor. Trate como incidente de segurança se algo escapar. Use as `seções` de provenance/anti-fake do motor para garantir que a IA do cliente **não cite nem invente** dado iGreen residual.
...
- **Fontes:** trocar matviews/queries iGreen pelas do schema de varejo (vendas, estoque, NF-e entrada, validade).
...
- [ ] Strip iGreen do DataScience completo (seção 4) + CI gate verde.
...
- [design-system-rebrand.md](design-system-rebrand.md) — o outro fork (DS visual): mesma disciplina de strip iGreen.
