...
| [uNF.md](dossiers/retaguarda/uNF.md) | **Nota Fiscal** (tela-coroa) | **F1+F2+F3 verdes** | **A maior tela do ERP** (uNF.pas 18.262 linhas; NF 209 col/NF_PROD 193 col/~40 tabelas). Recon (5+2+2 inspeções) + 6 auditorias adversariais. **F1** cadastro (SEM efeitos); **F2** fiscal por item (ICMS/ST/IPI reusando `precificacao`, puro — corrigiu dupla-redução de base); **F3** processamento: `processar`/`reverter` movem `ESTOQUE.QTDE` em **transação atômica** (entrada soma/saída baixa, **negativo bloqueia**, idempotente CAS, reverter bloqueado se SEFAZ; kardex `historico_prod`). Verde: API 123, web 25, **smoke 90/0**. ✅ **Risco lost-update do saldo RESOLVIDO** (engine ganhou `preservar`/`chaveNatural`: o cadastro de Produto preserva `estoque.qtde` do banco, owned pelo movimento). F4 financeiro; F5 contábil; F6 SEFAZ. Adiado/relaxado documentado com procedência |
