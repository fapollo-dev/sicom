...
| [UCadProduto.md](dossiers/retaguarda/UCadProduto.md) | **Produto** (hub do ERP) | em-revisão | 213 col / 43k linhas / 40 tabelas FK. **F1+F2 verdes**: produtos + cód. auxiliares + lookups + config fiscal + EAN; **MULTI_PRECO (preço/custo por empresa) na MESMA form**, com "Calcular venda" reusando `precificacao`. Tela NÃO calcula (motor reusado). F3 estoque / F4 kit·nutri / F5 replicação+golden pendentes |
