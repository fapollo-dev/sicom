...
| [UCadProduto.md](dossiers/retaguarda/UCadProduto.md) | **Produto** (hub do ERP) | em-revisão | 213 col / 43k linhas / 40 tabelas FK. **F1+F2+F3 verdes**: produtos + cód.auxiliares + lookups + config fiscal + EAN; **MULTI_PRECO** (preço/custo por empresa, "Calcular venda" reusa `precificacao`); **ESTOQUE** (saldo por empresa, na mesma form — saldo read-only/movido por transação, edita mín/máx/local). Tela NÃO calcula (motor reusado). F4 kit·nutri / F5 movimentação+replicação+golden pendentes |
