...
| [UCadProduto.md](dossiers/retaguarda/UCadProduto.md) | **Produto** (hub do ERP) | em-revisão | 213 col / 43k linhas / 40 tabelas FK. **F1–F4 verdes**: núcleo+cód.aux+lookups+fiscal+EAN; **MULTI_PRECO** (preço/empresa, reusa `precificacao`); **ESTOQUE** (saldo read-only, edita mín/máx/local); **kit/BOM** (composição/decomposição/receita — decomp 100%, flags derivadas, bloqueio desativar componente). Tela NÃO calcula. F4b nutricional/logística · F5 movimentação+replicação+golden pendentes |
