...
| [UCadProduto.md](dossiers/retaguarda/UCadProduto.md) | **Produto** (hub do ERP) | em-revisão | 213 col / 43k linhas / 40 tabelas FK. **F1 (núcleo) verde**: produtos + códigos auxiliares + lookups (unidade/famílias/alíquota) + config fiscal + validador EAN. Tela NÃO calcula (reusa `precificacao`). F2 preço (MULTI_PRECO)/F3 estoque/F4 kit·nutri/F5 replicação+golden pendentes |
