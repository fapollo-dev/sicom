...
| [uNF.md](dossiers/retaguarda/uNF.md) | **Nota Fiscal** (tela-coroa) | **F1+F2 verdes** | **A maior tela do ERP** (uNF.pas 18.262 linhas; NF 209 col/NF_PROD 193 col/~40 tabelas). Recon (5+2 inspeções) + 4 auditorias adversariais. **Dois eixos de estado** (PROC + STATUSNFE); **estoque movido por trigger Oracle no flip PROC N→S** (F3); financeiro ARECEBER/APAGAR (F4). **F1** = cadastro (header+itens+config fiscal+travas, **SEM efeitos**). **F2** = cálculo fiscal por item de entrada (ICMS próprio + ICMS-ST clássico + IPI) **reusando `precificacao`** via `POST /fiscal/nf/recalcular` (puro, não grava); auditoria pegou e corrigiu dupla-redução de base + bonificação-sem-ST (com regressão). Verde: API 123, web 25, **smoke 82/0**. F3 estoque; F4 financeiro; F5 contábil; F6 SEFAZ. Adiado/relaxado documentado com procedência |
