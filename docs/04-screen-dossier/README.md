...
| [uNF.md](dossiers/retaguarda/uNF.md) | **Nota Fiscal** (tela-coroa) | **F1 verde** | **A maior tela do ERP** (uNF.pas 18.262 linhas; NF 209 col/NF_PROD 193 col/~40 tabelas). Recon (5 inspeções) + 2 auditorias adversariais. **Dois eixos de estado** (PROC + STATUSNFE); **estoque movido por trigger Oracle no flip PROC N→S** (F3); financeiro ARECEBER/APAGAR (F4); fiscal reusa `precificacao` (F2). **F1 ENTREGUE e verde** (cadastro: header+itens+config fiscal+referências+máquina de travas, **SEM efeitos** — provado no smoke): `025_nf.sql` (nf/nf_prod/nf_referencia + lookups situacao_nf/cfop + UNIQUE parcial), aggregate empresaScoped (derivar=Σ totais, validar=travas+duplicidade), tela parametrizada Entrada/Saída. F2 fiscal; F3 estoque; F4 financeiro; F5 contábil; F6 SEFAZ/NFe. Adiado/relaxado documentado com procedência |
