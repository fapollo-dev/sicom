...
**Nota de honestidade sobre "golden" (NC-1/NC-2 do auditor de processo):** o verde da NF é o `smoke.ts` (ponta-a-ponta contra Postgres embarcado) com **valores esperados capturados manualmente do Oracle e fixados como expectativa** — prova auto-consistência e determinismo do novo, **não** um confronto reproduzível legado×novo. Os números "**5000/5000**" (chaves) e "**500/500**" (ICMS) são **confrontos manuais únicos, não asserts regressáveis** (o teste ancora 1–3 casos). Um `nf.parity.spec.ts` data-driven (fixtures por caminho condicional) fica **registrado como pendência** (`docs/06-testing-quality/parity-harness.md`).
