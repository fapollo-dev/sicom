...
| Campo | Valor |
|---|---|
| **Status** | corte-1 ENTREGUE e verde (F1–F6 + F2b/c + F5b) — lógica de negócio in-repo migrada; infra externa (SEFAZ real/DANFE/e-mail/NFC-e) e refinos golden-backed adiados com procedência |
| **Autor (migração)** | Claude (agente de migração), sessão 2026-06/07 |
| **Revisor (code-review de certificação)** | 5 agentes adversariais independentes (eventos/componentes · validações · efeitos stateful · cálculos fiscais-contábeis · conformidade de processo), 2026-07-02 — quem revisou ≠ quem escreveu cada fase |
| **Versão** | v2 (certificação 2026-07-02) |
| **Data** | 2026-07-02 |
| **Commit do legado analisado** | `/Library/SicomGit/retaguarda-master` (uNF.pas 18.262 L / udmNF.pas 12.103 L) |
| **Grau de paridade** | **paridade de RESULTADO verde in-repo + golden pontuais confrontados manualmente** (ver nota abaixo — NÃO é um parity-harness data-driven legado×novo) |

### Certificação de cópia fiel — code-review sênior (2026-07-02)

Revisão adversarial de ~320 handlers, todas as validações, todos os efeitos stateful e todos os cálculos, confrontados com o fonte legado e o Oracle golden (PINHEIRAO). **Núcleo transacional 100% FIEL-ENTREGUE** (nenhum stub vazio); adiados majoritariamente registrados. **5 correções aplicadas** nesta certificação:

1. **[ALTA] `reverter` não barrava NF FATURADA** → deixaria título ARECEBER/APAGAR órfão (legado `ReverteProcessamento` desfaz o financeiro junto, `uNF.pas:9000-9002`). Guard `faturada='S' → 422 NF_TEM_FATURAMENTO` (estornar o faturamento antes libera). Smoke §21.7.
2. **[ALTA] Trava de exclusão "referenciada por outra NF" ausente** (`uNF.pas:4145`) — a tabela `nf_referencia` já existia; comentário afirmava falsamente registro no §10. Implementada (`validarRemocao` → 422 `NF_REFERENCIADA`) + comentário corrigido. Smoke §21.8.
3. **[MÉDIA] Índice `ux_nf_natural` divergente da tupla-golden** — usava `tipo` (E/S) em vez de `tipoemissao`. Migration `042` alinha a `(nronf, serie, modelo, idempresa, tipoemissao, codparceiro)`, casando com o hook `validar` e o golden (`uNF.pas:4735/4761`).
4. **[MÉDIA] Partida contábil PRINCIPAL fragmentava por centro de custo** — o DIÁRIO real **consolida por situação** (golden CODNF 72296/84938/80589: N centros de custo → 1 linha, valor=Σ, codcc nulo). Passou a consolidar por `(situação, contadebito, contacredito)` resolvidos — colapsa 'F' multi-CC em 1 linha (fiel) e preserva o débito automático 'A' quando difere por CC. Smoke §29a2.
5. **[BAIXA] Hardcode `situacao=873` (CMV)** → constante nomeada `CMV_SITUACAO` com procedência (`CONFIG_INTEGRACAO_CONTABIL.CONFIG_CUSTO_NF_VENDA`), pendente da camada de config chave-valor.

**Nota de honestidade sobre "golden" (NC-1/NC-2 do auditor de processo):** o verde da NF é o `smoke.ts` (ponta-a-ponta contra Postgres embarcado) com **valores esperados capturados manualmente do Oracle e fixados como expectativa** — prova auto-consistência e determinismo do novo, **não** um confronto reproduzível legado×novo. Os números "**5000/5000**" (chaves) e "**500/500**" (ICMS) eram **confrontos manuais únicos, não asserts regressáveis**.

**Atualização 2026-07-02 — parity-harness `nf.parity.spec.ts` ENTREGUE (parcial, chave/DV):** `packages/shared/test/nf.parity.spec.ts` + fixture `nf.parity.golden.json` congelam **25 chaves de acesso REAIS** (NF.CHAVENFE autorizadas, 8 séries) capturadas do Oracle e, para cada uma, exigem que o migrado reproduza o legado por 3 caminhos independentes: `chaveNfeValida` (validade), `dvChaveNfe` (recomputo do DV — direção/pesos) e `montarChaveNfe` (roundtrip do layout). **77 asserts regressáveis** substituem o comentário "5000/5000" (re-conferido na captura: **0/5000 divergem** com a regra mód-11 correta — DV=11−resto, ≥10→0; um bug de transcrição do *verificador de captura* que zerava DV=1 foi pego e descartado, o `dvChaveNfe` migrado está correto). **Escopo restante do harness (registrado):** paridade de VALOR FISCAL por-item (TOTALNF/ICMS a partir de NF_PROD) NÃO cabe em golden real por-item porque `NF_PROD.VRVENDA` está frequentemente zerado na base (o legado deriva o total da ENTRADA de VRCUSTO — INFIDELIDADE-3); fica coberta pelo smoke §29/§29a2 (DIÁRIO contra golden pontuais + consolidação) e pela reconciliação de VRCUSTO no cutover.

### GAPs BAIXA registrados nesta certificação (nada perdido) — ver §10
...
- **TOTALNF** omite addends `TOTALIPI_DEVOLUCAO/TOTAL_FCP_VALOR_ST/VALORSERVICO/VROUTROS/ICMSDESON` (golden: 19/21 batem; 2 divergem exatamente por FCP-ST/IPI-dev) — acompanha a migração de FCP-ST/serviço.
- **`CancelarNFeManual` (uNF:6839)** — cancelamento sem webservice; verificar no cutover se o `cancelar` migrado cobre o caminho manual (hoje exige `statusnfe='P'`).
- **Teclado da NF (ADR-010)** sem E2E dedicado — mapa extraído (§9), teste Playwright do `NfCadMaster` pendente.
...
**Conformidade ADR-012 (FECHADA):** o **golden foi capturado e confrontado** (PINHEIRAO, V$SQL acessível) — ver "### Golden — confronto e correções". §4 SQL-por-query `[runtime]` (V$SQL real), §8 mapa de teclado (do .dfm) e §9 casos golden estão registrados ali. Status: **paridade de resultado verde in-repo + golden pontuais confrontados manualmente** (corte-1). ⚠️ Correção do rótulo anterior "certificada por golden" (cert. 2026-07-02, NC-1/NC-2): NÃO existe parity-harness data-driven legado×novo — o verde é smoke com expectativas capturadas manualmente do Oracle; um `nf.parity.spec.ts` fica pendente. Resíduos (deep-fiscal) ficam como F2b/F3b/F4b golden-backed, abaixo.
...
**F5b — DIÁRIO / partida dobrada ENTREGUE (corte-1, 2026-07-01):** fundou o razão contábil (`plano_contas`/`itens_integracao_contabil`/`lote_contabil`/`diario`, migration 035) + `empresas.integracao`/`plc.codcontabil`/`situacao_nf.nao_realiza_integracao`. `NfContabilizacaoService` (`POST /fiscal/nf/:id/contabilizar` e `/estornar-contabilizacao`): lançamento PRINCIPAL por situação — 1 linha `diario` com `CONTADEBITO`/`CONTACREDITO` pela `ITENS_INTEGRACAO_CONTABIL` (D+C por CODOPERACAO, TIPO='F'), valor=Σ `nf_contabil` da situação; gated por `EMPRESAS.INTEGRACAO='AUTOMATICA'` (udmNF:7778); idempotente (CAS em CONTABILIZADO); reversível (DELETE por CODORIGEM=12/IDORIGEM, UIntegracaoContabil:346); estorno no cancelamento se `contabilizado='S'` (uNF:6808). Spec+status: `uNF-F5b-contabil-diario.md`. Smoke §29. **Fase-2:** contas TIPO='A' (parceiro/PLC), linhas de imposto (ICMS/PIS/COFINS/CMV), auto-disparo no processar/transmitir, período fechado, situações não-mapeadas.

**Adiados de config do rateio (B, com procedência — nada perdido):** ~~DIÁRIO/partida dobrada~~ (ENTREGUE acima); **CX_APAGAR/CAIXA por centro de custo** (`GeraCxApagar`/`GerarLancamentosDeCaixa` — depende do rateio que a F5 entrega) → **F4b/F5b**; **auto-popular linhas** a partir de `SITUACAO_NF_PLC` (`InserirCentroDeCustosDefinidos`) e **restrição de CCs permitidos por situação** (`GetCentrosCustosPermitidos`) → **F5b** (refino UX); **`SituacaoDeBonificacao`/coluna `ADICIONAL`** (linha de adicional/bonificação) e **máx-1-linha em saída** → **F5b**; **coluna `CODCONTABIL`** em PLC (elo ao `PLANO_CONTAS` formal; 100% null no recorte) e **`PLANO_CONTAS` formal** → **F5b**; **Dia FECHADO** (`FECHAMENTO`, período contábil) [uNF.pas:4565] → **F5/fechamento**. **CX_APAGAR/CAIXA por centro de custo** (`GeraCxApagar`/`GerarLancamentosDeCaixa` — depende do rateio que a F5 entrega) → **F4b/F5b**; **auto-popular linhas** a partir de `SITUACAO_NF_PLC` (`InserirCentroDeCustosDefinidos`) e **restrição de CCs permitidos por situação** (`GetCentrosCustosPermitidos`) → **F5b** (refino UX); **`SituacaoDeBonificacao`/coluna `ADICIONAL`** (linha de adicional/bonificação) e **máx-1-linha em saída** → **F5b**; **coluna `CODCONTABIL`** em PLC (elo ao `PLANO_CONTAS` formal; 100% null no recorte) e **`PLANO_CONTAS` formal** → **F5b**; **Dia FECHADO** (`FECHAMENTO`, período contábil) [uNF.pas:4565] → **F5/fechamento**.
...
- **Chave de acesso / DV mód 11:** **5000 chaves reais confrontadas manualmente (confronto único, não regressável)** + **3 ancoradas como assert no teste** (`chave-nfe.spec.ts`): `31200866312653000114550010005599791528020227` (DV 7), `…815729057` (DV 7), `…045077624` (DV 4). Chave é **nua (44 díg, sem prefixo 'NFe')** — como o migrado guarda. Layout cUF/AAMM/CNPJ/mod/serie/nNF/tpEmis/cNF/DV confirmado (SUBSTR bate com MODELO/SERIE/NRONF).
...
- **SENAR (8ª retenção da aba Retenções) — SEM motor no legado (confronto visual 2026-07-02):** a tela mostra `Total SENAR`/`Alíq. SENAR` e a coluna `NF.TOTAL_RET_SENAR` existe, mas **nenhuma unit Delphi calcula SENAR** (`grep -rail senar` em todo o fonte = 0) e a coluna é **morta no golden** (1 NF em 23.408; FUNRURAL/ISSQN/INSS = 0/23.408 — este cliente não usa retenções na prática). Já não-modelado na migração Parceiros (019_parceiros_fiscal.sql). **Nada a migrar** (não há regra); se um dia replicar a tela 1:1, é só o campo de UI. As 7 retenções migradas (A1) são exatamente as que o motor `CalcularRetencoes` (udmNF:3558) computa.
- **Registrado na certificação 2026-07-02 (ver §0 para procedência e severidade):** peso agregado por item sob `CALCULAPESO='S'` (`uNF:8519`); `DESC_ACRE_MEDIO` na importação de venda (`uNF:6596`); trava de exclusão por **devolução de compra emitida** (`uNF:4176`, módulo de compras não migrado — a trava por *referência de outra NF* JÁ foi implementada nesta cert.); base fiscal de ENTRADA sobre VRCUSTO (INFIDELIDADE latente); gate `LIBERA_DIGITACAO_NF_TERCEIROS`; `SEQUENCIA_NFE<>'N'`/`lpad(serie,3)` na auto-numeração; addends do TOTALNF (IPI-dev/FCP-ST/serviço/ICMSDESON); caminho `CancelarNFeManual` (sem webservice); **parity-harness `nf.parity.spec.ts`** — chave/DV ENTREGUE (25 golden reais, 77 asserts); fiscal por-item resta (limitado por VRVENDA/INFIDELIDADE-3, coberto por smoke §29); **E2E de teclado do `NfCadMaster`** (NC-4/ADR-010).
