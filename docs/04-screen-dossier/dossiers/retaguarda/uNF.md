...
**Adiado (F2c fase-2):** precificação-lê-empresa (simplesNacional/despOperacional/modoMargem — é a tela Produto, `uMargemPreco.pas:123`), **figura fiscal por catálogo** (seleção multi-chave INDEXADOR + flag FIGURAFISCAL 'D' não-consulta / 'O'-'S' consultam), **crédito de entrada SN** (ALQSIMPLESNAC), **ST_da_nota** (o `calcularIcmsSt` não subtrai `GetVrICMSSTNotaFiscal` — só relevante em NF importada com ST → F6/importação), Lei 3166/DIFAL/FCP-ST.

**Dívida do ciclo de vida (validação cross-cutting 2026-06-30 — `docs/06-testing-quality/validacao-cross-cutting-2026-06.md`):** (a) **cancelar não estorna financeiro/contábil** — NÃO é bug: `TfrmNF.CancelarNFE`(uNF.pas:6773)→`CancelaFaturamento`(6668) é **gated por `ESTORNA_FINANCEIRO_NF`** (default golden 'N') → o legado também não deleta títulos por padrão (só `AdicionaPendenciaFinanceiro`); o migrado espelha o default. Orquestração completa (delete config-gated + guarda `VerificaExisteBaixas` + pendência + estorno contábil `TIntegracaoContabil.Estornar` se `CONTABILIZADO='S'`+`INTEGRACAO='AUTOMATICA'`, uNF.pas:6808) → **F4b/F5b**. (b) **denegada `statusnfe='D'` encalha estoque** (reverter bloqueia em D; cancelar exige P) — inalcançável no corte-1 (simulador só sucesso) → **F6b**. (c) **`faturar` não exige `proc='S'`** (assimétrico com transmitir) → avaliar em **F4b**. (d) **sem gate de reconciliação no `processar`** (recalcular é puro; legado revalida o total) → **F3b**.
