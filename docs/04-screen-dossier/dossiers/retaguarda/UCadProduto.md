...
>
> **MOTOR COMPLETO (corte precificação, 2026-07-10) — ENTREGUE e verde.** Adicionadas ao `FiscalPricingService` as 3 peças que faltavam (fórmulas de `uPrecificacaoProdutos.pas`, verificáveis por teste): **`custoLiquido`** = (custo+ST+IPI+frete+seguro+despac) − créditos PIS/ICMS (:1314); **`pmz`** = custoFinal/(1−(PIS_sai+COFINS_sai+ICMS+FCP+DESPOP)/100) (:1333); **`margemLiquida`** = cadeia venda_líq→lucro_bruto→(−despesa)→IR/CSLL sobre o lucro-após-despesa (dbtLucroL, :1139-1152)→lucro_líq→margem%. `precificarProduto` agora precifica **sobre o CUSTO LÍQUIDO** (fold ALTA — o legado usa VRCUSTOLIQUIDO como base do markup, uMargemPreco:1318; sem componentes = custo, retrocompat) e retorna custoLiquido/pmz/lucroBruto/lucroLiquido/margemLiquida. **PMZ tolerante:** saídas≥100% (não-precificável) NÃO derruba o valorVenda (pmz=0; fiel ao legado que exibe a venda mesmo com PMZ absurdo). Front `ProdutoCadMaster` mostra o painel (custo líq/PMZ/margem/lucro) + preenche `margeml`. **2 auditores: 0 ALTA final.** FRONTEIRA: análise de margem só no modo LÍQUIDO (o modo 'final' embute IR/CSLL no gross-up — adiado); o front ainda passa PIS/COFINS=0 (derivação do PISCOFINS de saída adiada, como já era no valorVenda). **DESCOBERTA (Oracle):** os valores de preço GRAVADOS no item do pedido/produto são snapshot manual/histórico (fórmulas batem 0,4%) → o corte é o MOTOR (calc puro verificável), NÃO reproduz os 286k snapshots.
>
