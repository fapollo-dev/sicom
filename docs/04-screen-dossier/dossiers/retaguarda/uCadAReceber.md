...
| Campo | Valor |
|---|---|
| **Status** | corte-1 (cadastro/gestão) + corte-2 (baixa núcleo) ENTREGUES 2026-07-02; corte-3a (baixa PARCIAL, 054) + corte-3b (CONTÁBIL da baixa DINHEIRO, 055) ENTREGUES 2026-07-04; **corte-3c (recurso BANCO na baixa, 058)** ENTREGUE e verde 2026-07-06 (smoke 382/0), AR+AP + front. Recon por 3+ agentes; cada corte auditado (adversarial 2 agentes: paridade vs Oracle real + não-regressão). |
...
- **Corte-2 (baixa núcleo) — ENTREGUE (migration 044):** tabela `areceber_bx` (INDR estorno-lógico) + serviço stateful `areceber-baixa.service.ts` (baixar: juro+multa+acre_desc → `valorpg`, quita via CAS; estornar: INDR='E' reabre, NÃO deleta) no molde `nf-faturamento` (forUpdate+CAS+tenant codempresa). Juros default = view `get_areceber.juro` (fórmula legada). Guardas: já-baixado/agrupado/em-lote(`itens_lotecob`)/não-baixado/baixa-contabilizada. Endpoints `POST cadastro/areceber/:id/baixar|estornar-baixa`. Front `BaixaSection`. Smoke §32 (8 casos: quita, dupla-baixa, estorno-lógico, juros/desconto, agrupado, em-lote, IDOR). **Adiado:** período-fechado na baixa (corte-3).
- **Corte-3a (baixa PARCIAL) — ENTREGUE (migration 054, 2026-07-04):** quando `valorpg < total` (total = valor+juros+multa+acréscimo/desconto, `UBaixaAreceber.pas:1448`), o título original é quitado E um NOVO título com o SALDO (`total − valorpg`) é gerado (`ORIGEM='B'`, `GERADO='SISTEMA'`, `QUITADA='N'`, `TIPODOC='DUPLICATA'`, `UBaixaAreceber.pas:1403/1449/1456`). Vínculo baixa↔saldo por FK `areceber_bx.codrcb_gerado` (AP: `apagar_bx.codapg_gerado`) — mais robusto que o `OBS LIKE` do legado (`UReversaoBaixaContasReceber.pas:44`). O estorno da baixa **deleta o saldo** (como o legado, `DELETE_RCB_PARCIAL`), mas SÓ se o saldo estiver INTOCADO — qualquer baixa no saldo (ativa OU estornada), agrupamento ou lote bloqueia (`REVERSAO_PARCIAL_SALDO_BAIXADO`/`TITULO_AGRUPADO`/`TITULO_EM_LOTE`), evitando `caixa_mov` órfão e perda de histórico. Caixa (DINHEIRO) recebe só o `valorpg`. Espelhado no A Pagar (gêmeo). Smoke §43 (12 casos AR+AP: parcial gera saldo, total não, tipodoc/cad_manual/dtvenc, valorpg>total, estorno remove saldo, estorno bloqueado, IDOR-safe via codempresa). **Divergências conscientes vs legado (auditoria paridade):** (a) **DTVENC do saldo = data da baixa** (default `dtpgto`; opcional `dtvencSaldo`) — o legado prompta uma data nova (`:1433`; Oracle confirma DTVENC=dia da baixa, não o vencimento original), NÃO herda o venc original; (b) **`cadastrado_manualmente='N'`** (DEFAULT) = "não-manual" — o legado grava NULL; o monorepo remapeia NULL→'N' consistentemente para SISTEMA (`043_areceber_gestao.sql:45`); (c) **`valorpg > total` é REJEITADO** (`TITULO_VALOR_EXCEDE`) — o legado gera TROCO/crédito (`MOSTRAR_TROCO_BAIXA_CR`, `:1499`); troco é corte-3, até lá rejeita para não gravar recebimento fantasma; (d) **`multa` entra na base do saldo** — extensão herdada do corte-2 (default 0); o legado A Receber não calcula multa na baixa; (e) **OBS** referencia "título" (modelo 1-título) em vez de "lote" — inofensivo (estorno usa FK). **Adiado p/ corte boleto:** `NRODUP`/`DUPLICATA`/`IDPGTO`(FORMAS_PGTO DESTINO='RCB')/`DOCNF` do saldo (consumidor = emissão de boleto/PDV, também adiado).
- **Corte-3b (CONTÁBIL da baixa DINHEIRO) — ENTREGUE (migration 055, 2026-07-04):** `BaixaContabilService` (molde `caixa-contabil`/`nf-contabilizacao`) lança 1 partida BALANCEADA no DIÁRIO ao baixar com recurso **DINHEIRO** (auto-disparo best-effort na mesma transação da baixa — engole BusinessRuleError, fiel ao legado que só avisa no log). **AR (CODORIGEM=16, situação 2009):** D 183 CAIXA CENTRAL / C cliente (`PARCEIROS.CODCONTABIL`). **AP (CODORIGEM=15, situação 2004):** D fornecedor (`PARCEIROS.CODCONTABIL_FOR`) / C 183 CAIXA. Gate `EMPRESAS.INTEGRACAO='AUTOMATICA'`, `assertPeriodoAberto` (fail-open), idempotente (`*_bx.contabilizado`), reversível (DELETE por CODORIGEM/IDORIGEM=codrcbbx/codapgbx + lote órfão). O estorno da baixa passou a **reverter o contábil** em vez de bloquear (removido o antigo `BAIXA_CONTABILIZADA` — espelha NF reverter/caixa reabrir). **Confirmado no Oracle real:** CONFIG_BAIXA_RCB=2009/APG=2004; C-leg cliente (hist 93) = **VALORPG exato em 8522/8522 linhas** (inclui desconto/multa embutidos — o cliente é creditado o efetivamente recebido, NÃO só o principal); CODORIGEM 16/15 só têm codoperacao 2009/2004 (sem linha separada de juros dentro da origem da baixa); idorigem=codrcbbx/codapgbx bate 1:1. **Divergências CONSCIENTES (2 auditores):** (a) **partida BALANCEADA por baixa** vs legado single-legged-agregado (Σ preservado; AP agrega por lote em 122/17.336 linhas); (b) **só recurso DINHEIRO** → money leg fixa 183 (cobertura ~8% AR/~5% AP; o resto é cartão 213/bancos 186/190/195 → **corte-2** faz a perna por RECURSO). **Endurecimento (achado #2):** o serviço exige EXATAMENTE 1 perna 'F' (dinheiro) — se a IIC 2009/2004 for reimportada com os 2 lados 'A' (recurso-driven), PULA em vez de gerar D=cliente/C=cliente (smoke §44.7). Smoke §44 (7 casos AR+AP: D/C+valor, estorno reverte, período fechado pula, não-AUTOMATICA pula, guarda 'A'/'A'). **Adiado (corte-2):** perna por RECURSO (banco/cartão/cheque → 186/213/195…), linhas separadas de JUROS/MULTA/DESCONTO (situações 874/875/878/879/1150), baixa por cartão (893).
- **Corte-3c (recurso BANCO na baixa) — ENTREGUE (migration 058, 2026-07-06):** a baixa/pagamento por **depósito bancário** (`recurso='BANCO'` + `codconta`). O contábil usa a conta contábil do banco (`contas_bancarias.codlanccontabil`) como perna de dinheiro, em vez da 183 CAIXA: **AR → D <banco> / C cliente (211)**; **AP → D fornecedor (CODCONTABIL_FOR) / C <banco>**; mesma situação **2009/2004**, CODORIGEM **16/15** (só troca a conta D fixa 183 → conta do banco). BANCO **NÃO toca o caixa** (é depósito, não espécie — sem `caixa_mov`, sem exigir caixa aberto). **Confirmado no Oracle:** os débitos da baixa AR se distribuem por recurso (186 BANCO ITAÚ / 190 CEF / 195 BB), = `contas_bancarias.codlanccontabil`, situação 2009. `baixa-contabil.contabilizarNoTrx` ganhou `contaMoney` (substitui a 183 na perna 'F'); `baixarTituloSchema` ganhou `recurso: 'DINHEIRO'|'BANCO'` + `codconta` (superRefine: BANCO exige codconta → 400; conta inexistente → 422 `CONTA_BANCARIA_NAO_ENCONTRADA`). Front: seletor de Recurso (Dinheiro/Banco) + lookup de conta bancária na baixa AR/AP. Migration 058 (plano_contas 186 BANCO ITAU + vincula contas_bancarias(1).codlanccontabil='186'). **2 auditores (paridade vs Oracle + não-regressão): 0 ALTA/MÉDIA de código.** Correções: seed 186 `tipo='E'` (alinhado a 053/046, era 'A'); `contaBanco` best-effort — pula o contábil (não aborta a baixa) se o banco não tem `codlanccontabil` ou aponta p/ conta ausente no plano. Divergência CONSCIENTE (herdada): partida-balanceada por baixa vs legado single-legged (forçada pelo `diario.contadebito/contacredito NOT NULL`). Smoke §46 (4 casos AR+AP). **MCB NÃO é lacuna** (Oracle: baixa não grava em `mov_contas_bancarias`; o razão do banco é reconciliado por OFX/fechamento). **Adiado:** cheque/cartão (tabelas CHEQUE/CARTAO/CARTAO_BX + situação 893/CODORIGEM 51 ausentes); juros/desconto separados = INÓCUO (o parceiro recebe o valorpg cheio; 878/879 vivem sob codorigem 54/57/58).
- **Corte-3 (adiado/registrado):** agrupamento in-place (AGRUPARECEBER), demais recursos de baixa (cheque/cartão/permuta/saldo/troco → dependem de CHEQUE/CARTAO/caixa multi-forma não migrados), boleto/CNAB, adiantamento, pendências, senha ADM/desconto, geração multi-parcela na tela. BUG legado `AtualizaValoresAgrupamento` (TOTAL derivado de VALOR) a decidir no corte-3.
