# F5b — Contábil DIÁRIO / partida dobrada (NF) — SPEC + corte-1 ENTREGUE

> **Status:** `corte-1 + fase-2 ENTREGUES e verdes` (2026-07-01). **Corte-1 (035):** fundou o razão contábil (`plano_contas`/`itens_integracao_contabil`/`lote_contabil`/`diario` + `empresas.integracao`/`plc.codcontabil`/`situacao_nf.nao_realiza_integracao`); lançamento PRINCIPAL por situação (TIPO='F') via endpoints EXPLÍCITOS `contabilizar`/`estornar-contabilizacao`, gated `EMPRESAS.INTEGRACAO='AUTOMATICA'`, idempotente (CAS), reversível, + estorno no cancelamento. **Fase-2 (036):** contas AUTOMÁTICAS **TIPO='A'** (crédito=parceiro `CODCONTABIL_FOR`/`CODCONTABIL`; débito=ponte PLC `CODCC→PLC.CODCONTABIL`) + linhas de imposto **PIS/COFINS** (situação do CFOP; base=TOTALNF × 1,65%/7,6% LR não-cumulativo — **golden NF 72044/71822 exato**). Rateio agrupado por (situação, centro de custo). Smoke §29 (principal 'F' + 2x→422 + estorno + guardas + 'A' D148/C11141 + PIS/COFINS 1,65/7,60). Verde: api tsc 0, api test 123, **smoke 168/0**. **Fase-3 (abaixo):** linha de ICMS próprio, CMV (saída), auto-disparo no processar/transmitir, período fechado, PC_CONFIG (base/rate por regime), situações não-mapeadas.
