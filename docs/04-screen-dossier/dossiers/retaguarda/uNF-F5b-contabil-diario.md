...
> **Status:** `corte-1 ENTREGUE e verde` (2026-07-01) — fundou o razão contábil (`plano_contas`/`itens_integracao_contabil`/`lote_contabil`/`diario` + `empresas.integracao`/`plc.codcontabil`/`situacao_nf.nao_realiza_integracao`, migration 035). **Entregue:** lançamento PRINCIPAL por situação (1 linha DIARIO com CONTADEBITO/CONTACREDITO pela IIC, TIPO='F') via endpoints EXPLÍCITOS `POST /fiscal/nf/:id/contabilizar` e `/estornar-contabilizacao` (`NfContabilizacaoService`), gated por `EMPRESAS.INTEGRACAO='AUTOMATICA'`, idempotente (CAS em CONTABILIZADO), reversível (DELETE por CODORIGEM=12/IDORIGEM), + estorno no cancelamento (se `contabilizado='S'`). Smoke §29 (contabilizar D148/C11141 valor 50 + 2x→422 + estorno + guardas). **Fase-2 (abaixo):** contas TIPO='A' (parceiro/PLC automático), linhas de imposto (ICMS/PIS/COFINS/CMV), auto-disparo no processar/transmitir, período fechado, situações não-mapeadas.
