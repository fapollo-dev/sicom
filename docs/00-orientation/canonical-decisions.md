...
## ADR-015 — Contrato único de erro (envelope PT) + validação zod/BR; nunca 500 genérico
- **Decisão:** Toda resposta de erro da API é **um envelope só** — `ErroResposta { statusCode, code (estável, MAIÚSCULAS_SNAKE), message (PT, motivo real), campos?[] }`. Validação é **zod**, com **validadores BR** para os campos conhecidos (CPF/CNPJ/celular/e-mail/CEP/UF — normalizam máscara + validam dígito/checksum, mensagens PT). **NUNCA** 500 genérico "erro no servidor": o status é **ajustado** ao problema e a `message` traz o **motivo real** em PT. Erros de banco são mapeados (**FK `23503`→409 `REGISTRO_RELACIONADO_INEXISTENTE`; unique `23505`→409 `DUPLICADO`; not-null `23502`→400 `CAMPO_OBRIGATORIO`; check `23514`→422 `REGRA_VIOLADA`**). O front **consome o envelope** e exibe num **modal de mensagens padrão** (componente tratado: `MensagemProvider`/`useMensagem` + Modal/AlertModal do DS).
- **Porquê:** O operador entende o **motivo real em PT**; os erros viram **acionáveis** (campo grifado, mensagem clara); **nada** de stacktrace ou 500 cru vazando; e há **consistência back↔front** via contrato único (mesmo shape, mesmo zod dos dois lados).
- **Implicação:** O filtro global de exceção mapeia **tudo** num só ponto; o pipe zod gera `campos[]`; os validadores BR moram em `@apollo/shared`; o modal padrão é a única forma de exibir erro no front. Contrato real: `packages/shared/src/erro.ts` (`ErroResposta`, `isErroResposta`) e `packages/shared/src/validators/br.ts`. Detalhe e tabela de mapeamento em [../02-stack-and-standards/api-erros-e-validacao.md](../02-stack-and-standards/api-erros-e-validacao.md).

## ADR-016 — Telas de cadastro: pilar CadMaster (record-first) + engine declarativo
